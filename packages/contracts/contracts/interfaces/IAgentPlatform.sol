...
/// @notice Interface for the Somnia Agent Platform precompile.
/// @dev Testnet: 0x037Bb9C718F3f7fe5eCBDB0b600D607b52706776
///      Mainnet: 0x5E5205CF39E766118C01636bED000A54D93163E6
interface IAgentPlatform {
    // ─── Enums ─────────────────────────────────────────────────────────────────

    enum ConsensusType {
        Majority,  // Finalize when threshold validators return identical results
        Threshold  // Finalize when threshold validators respond (any result)
    }
    ...
    /// @notice Create a standard request (subcommitteeSize=3, majority consensus, default timeout)
    ...
    /// @notice Create an advanced request with custom parameters
    ...
    /// @notice Minimum deposit required for standard request (operations reserve only)
    /// @dev Add (perAgentPrice × subcommitteeSize) on top for actual agent budget
    function getRequestDeposit() external view returns (uint256);

    /// @notice Deposit for advanced request with custom subcommittee size
    function getAdvancedRequestDeposit(uint256 subcommitteeSize) external view returns (uint256);
}

/// @notice Callback interface — implement this in contracts that receive agent results
...
/// @notice Payload encoding helpers for Somnia base agents
